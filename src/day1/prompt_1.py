...
the mass of the added fuel? (Calculate the fuel requirements for each module separately, then add them all up
at the end.)


'''

# 4899730 is apparently too high


# part 1: fuel calcs for modules
def module_fuel_calc(mass):
    # fuel needed is (floor of mass divided by three) minus 2
...
def fuel_for_fuel_calc(mass):
    fuel_check = mass
    fuel_needed = 0
    while fuel_check > 0:
        fuel_check = module_fuel_calc(fuel_check)
        print(fuel_check)
        fuel_needed += fuel_check
        print(fuel_needed)
...
def part1():
    total_fuel_needed = 0
    module_fuel_needed = 0
    # initial fuel of all modules
    with open('prompt_1_input.txt') as input:
        # with open('prompt_1_test_3.txt') as input:
        for line in input:
            module_fuel_needed += module_fuel_calc(int(line))
    print(f'fuel needed for all modules: {module_fuel_needed}')
...
def part2():
    ...
    with open('prompt_1_input.txt') as input:
        # with open('prompt_1_test_3.txt') as input:
        for line in input:
            module_fuel_needed += fuel_for_fuel_calc(int(line))
    print(f'fuel needed for all modules: {module_fuel_needed}')
